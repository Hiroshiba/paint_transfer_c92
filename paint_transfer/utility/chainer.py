...
def eliminate_variable_recursive(obj):
    if isinstance(obj, tuple) or isinstance(obj, list):
        return [eliminate_variable_recursive(elem) for elem in obj]
    elif isinstance(obj, dict):
        return {key: eliminate_variable_recursive(obj[key]) for key in obj}
...
def dump_graph(root_name, out_name='cg.dot', variable_style=None, function_style=None):
    ...
    def dump_graph(trainer):
        var = [trainer.observation[n] for n in root_name]
